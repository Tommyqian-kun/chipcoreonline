/**
 * UPF工具多页面交互 - 初始化页面
 * 基于SDC多页面工具，修改为UPF工具的4文件上传和相关逻辑
 */
...
// UPF表单验证schema - 4个文件：hier.yaml, pvlog.v, pmobj.tcl, pcell.yaml
...
const UpfGeneratorInitializeThrpages: React.FC = () => {
    ...
    const onSubmit = async (data: InitializeFormValues) => {
        const { modName, version, isFlat, hierYamlFile, pvlogFile, pobjTclFile, pcellYamlFile } = data;
        ...
        try {
            ...
            // 2. 检查文件是否都已上传
            if (!hierYamlFile || !pvlogFile || !pobjTclFile || !pcellYamlFile) {
                throw new Error('请上传所有必需的文件：hier.yaml、pvlog.v、pobj.tcl、pcell.yaml');
            }
            ...
            if (!allFilesValid) {
                ...
                if (!fileValidationStates.pcellYaml.isValid) {
                    invalidFiles.push(`pcell.yaml (${fileValidationStates.pcellYaml.error})`);
                }
            ...
            // 调用初始化API - 使用项目统一的api实例，正确配置baseURL和credentials
            ...
            formData.append('pvlogFile', pvlogFile);
            formData.append('pobjTclFile', pobjTclFile);
            ...
            setTimeout(() => {
                navigate(`/tools/upf-generator/task/${result.taskId}`);
            }, 1000);
                            ...
                            <Form {...form}>
                                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                                                ...
                                                <FormItem>
                                                    ...
                                                    <FormControl>
                                                        <Select value={field.value} disabled={true}>
                                                            <SelectTrigger className="border-orange-300 bg-gray-100 text-gray-500 w-1/2">
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        {/* pcell.yaml上传 */}
                                        <FormField control={form.control} name="pcellYamlFile" render={({ field }) => (
                                            <EnhancedFileUpload
                                                title="上传pcell.yaml"
                                                name="pcellYamlFile"
                                                accept=".yaml,.yml"
                                                ...
                                                fileType="yaml"
                                                toolType="upf"
                                                onFileChange={(file) => form.setValue('pcellYamlFile', file)}
                                                onValidationChange={handleFileValidationChange('pcellYaml')}
                                                field={field}
                                            />
                                        )} />
                                    </div>
...
export default UpfGeneratorInitializeThrpages;
