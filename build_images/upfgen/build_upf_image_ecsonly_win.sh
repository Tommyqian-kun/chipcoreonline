...
# 工具脚本路径: /home/master/stone/proj/onlineEDA\test_data\tools_collection\upfgen
# 镜像存储路径: /home/master/stone/proj/onlineEDA\docker\images\upf
...
rollback_to_version() {
    ...
    if [[ $? -eq 0 ]]; then
        ...
        local version_file="$IMAGES_DIR/logiccore_upf-generator_${target_version}.tar"
        ...
        if [[ -f "$version_file" ]]; then
            ...
            warning "⚠ Version tar file not found: $version_file"
        fi
...
check_files() {
    ...
    local required_files=(
        "$DOCKERFILE_PATH"
        "$PROJECT_ROOT/test_data/tools_collection/upfgen"
...
# 创建存储目录
create_storage_dirs() {
    log "Creating storage directories for ECS Only Windows testing..."
    ...
    success "All storage directories ready"
}
...
build_image() {
    ...
    for file in "${required_build_files[@]}"; do
        if [[ ! -e "$file" ]]; then
            error "Required build file not found: $file"
            exit 1
        fi
    done
...
# 测试镜像
test_image() {
    log "Testing Docker image for ECS Only Windows testing..."

    # 根据交互模式确定测试镜像名称
    local test_image_name
    if [[ "$INTERACTION_MODE" == "multi" ]]; then
        test_image_name="$IMAGE_NAME-multi:$VERSION"
    else
        test_image_name="$IMAGE_NAME:$VERSION"
    fi
...
save_image_to_file() {
    ...
    if [[ "$INTERACTION_MODE" == "multi" ]]; then
        # 多页面交互模式
        full_image_name="$IMAGE_NAME-multi:$VERSION"
        image_file="$IMAGES_DIR/logiccore_upf-generator-multi_${VERSION}.tar"
        latest_file="$IMAGES_DIR/logiccore_upf-generator_latest.tar"  # latest文件名保持标准格式
    else
        # 单页面交互模式
        full_image_name="$IMAGE_NAME:$VERSION"
        image_file="$IMAGES_DIR/logiccore_upf-generator_${VERSION}.tar"
        latest_file="$IMAGES_DIR/logiccore_upf-generator_latest.tar"
    fi
...
Configuration:
  ...
  - Tool Scripts: /home/master/stone/proj/onlineEDA\test_data\tools_collection\upfgen
  - Image Storage: /home/master/stone/proj/onlineEDA\docker\images\upf

Examples:
  # Build single-page interaction version
  $0 v1.0.0 single             # Build standard version: logiccore/upf-generator:latest
  $0 v1.2.3 single             # Build standard version: logiccore/upf-generator:v1.2.3

  # Build multi-page interaction version
  $0 v1.0.0 multi              # Build multi version: logiccore/upf-generator:latest + logiccore/upf-generator-multi:v1.0.0
  $0 v1.2.3 multi              # Build multi version: logiccore/upf-generator:latest + logiccore/upf-generator-multi:v1.2.3
...
Output:
  - Docker image: logiccore/upf-generator-multi:VERSION (versioned)
  - Docker image: logiccore/upf-generator-multi:latest (linked to latest version)
  - Saved file: {IMAGES_DIR}/logiccore_upf-generator-multi_VERSION.tar
  - Saved file: {IMAGES_DIR}/logiccore_upf-generator_latest.tar

Storage:
  All images are saved as .tar files in the images directory for ECS deployment.
  The latest tag always points to the most recently built version.

Troubleshooting:
  - Check directory: ls -la $IMAGES_DIR
  - Verify Docker images: docker images $IMAGE_NAME
  - Test image load: docker load -i $IMAGES_DIR/logiccore_upf-generator_latest.tar
...
main() {
    ...
    log "Step 7/8: Verifying saved files..."
    verify_saved_image
    echo ""
    ...
    local latest_file
    if [[ "$INTERACTION_MODE" == "multi" ]]; then
        version_file="logiccore_upf-generator-multi_${VERSION}.tar"
        latest_file="logiccore_upf-generator_latest.tar"
    else
        version_file="logiccore_upf-generator_${VERSION}.tar"
        latest_file="logiccore_upf-generator_latest.tar"
    fi
    ...
    echo "  ✓ Version image saved to: $IMAGES_DIR/$version_file"
    echo "  ✓ Latest image saved to: $IMAGES_DIR/$latest_file"
    echo "  ✓ Storage directory: $IMAGES_DIR"

    # 最终验证
    log "Final verification:"
    if [[ -f "$IMAGES_DIR/$version_file" ]]; then
        local file_size=$(get_file_size "$IMAGES_DIR/$version_file")
        success "✓ Version image file exists (size: $file_size bytes)"
    else
        error "✗ Version image file missing!"
    fi

    if [[ -f "$IMAGES_DIR/$latest_file" ]]; then
        local file_size=$(get_file_size "$IMAGES_DIR/$latest_file")
    ...
    if [[ "$INTERACTION_MODE" == "multi" ]]; then
        echo "  4. Test multi-page interaction UPF tool through web interface"
    else
        echo "  4. Test single-page interaction UPF tool through web interface"
    fi
